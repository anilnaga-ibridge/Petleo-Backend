...
# def create_reset_token(phone_number, ttl=RESET_TOKEN_TTL_SECONDS):
#     token = str(uuid.uuid4())
#     cache.set(f"{RESET_TOKEN_CACHE_PREFIX}{token}", phone_number, timeout=ttl)
#     return token


# def get_phone_for_reset_token(token):
#     return cache.get(f"{RESET_TOKEN_CACHE_PREFIX}{token}")


# def delete_reset_token(token):
#     cache.delete(f"{RESET_TOKEN_CACHE_PREFIX}{token}")
...
# # ---------------------
# def is_same_calendar_day(dt1, dt2):
#     """Return True if dt1 and dt2 are same calendar day in server local time."""
#     if not dt1 or not dt2:
#         return False
#     d1 = timezone.localtime(dt1).date()
#     d2 = timezone.localtime(dt2).date()
#     return d1 == d2
...
#             except User.DoesNotExist:
#                 return Response({"detail": "Phone number not registered. Please register first."},
#                                 status=status.HTTP_404_NOT_FOUND)

#         # Rate limit using your util (may raise redis errors if redis is down)
#         if not increment_rate_limit(phone, RATE_LIMIT_WINDOW, RATE_LIMIT_MAX):
#             return Response({"detail": "Too many OTP requests. Try again later."},
#                             status=status.HTTP_429_TOO_MANY_REQUESTS)
...
#         # Correct PIN validity (uses fixed function)
#         "pin_valid_today": is_pin_valid_today(user),

#         # Should user set PIN right now? (passed from login handler)
...
# from users.utils import is_pin_valid_today
...
# #     # ===================================================================
# #     # EXISTING HANDLERS (NO CHANGE REQUIRED BELOW THIS LINE)
# #     # ===================================================================
...
#         # OTP is correct → remove OTP session
#         delete_otp_session(session_id)

#         # ===============================
#         # 1️⃣ RESET PIN FLOW (SPECIAL CASE)
#         # ===============================
#         if purpose == "reset_pin":
#             return Response({
#                 "message": "OTP verified. You can now reset your PIN.",
...
#     # ==========================================================
#     # KEEP ALL EXISTING HANDLER METHODS (NO CHANGE REQUIRED)
#     # ==========================================================
...
#         # Save PIN with correct local timezone
#         tz = timezone.get_current_timezone()
...
#             return Response({"detail": "phone_number and pin are required."}, status=status.HTTP_400_BAD_REQUEST)
...
#             return Response({"detail": "No PIN set. Please login via OTP and set a PIN."}, status=status.HTTP_400_BAD_REQUEST)

#         # Check if PIN is valid ONLY for today (fixed timezone-safe)
#         if not is_pin_valid_today(user):
#             return Response({"detail": "PIN expired. Please login using OTP."}, status=status.HTTP_400_BAD_REQUEST)

#         if not check_password(pin, user.pin_hash):
#             return Response({"detail": "Invalid PIN."}, status=status.HTTP_400_BAD_REQUEST)

#         # Success — update last_pin_login with correct local timezone
...
#         # PIN is only valid on same calendar day it was set
#         if not is_pin_valid_today(user):
#             return Response({"detail": "PIN expired. Please login using OTP."}, status=status.HTTP_400_BAD_REQUEST)
...
#         # success -> update last_pin_login with local timezone and issue tokens
#         user.last_pin_login = timezone.now().astimezone(timezone.get_current_timezone())
#         user.save(update_fields=["last_pin_login"])
...
#         response = {"message": "OTP resent successfully.", "session_id": session_id, "sms_sent": sms_ok}
#         if getattr(settings, "SMS_BACKEND", "console") == "console":
#             response["otp"] = otp
