...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            with transaction.atomic():
                ...
                lock_success = SlotLockService.lock_slot(employee_id, selected_time)
                if not lock_success:
                ...
                if BookingItem.objects.filter(
                    ...
                    SlotLockService.release_slot(employee_id, selected_time)
                ...
                success_url = self.request.build_absolute_uri(f'/booking/success/?booking_id={booking.id}')
                cancel_url = self.request.build_absolute_uri(f'/booking/cancel/?booking_id={booking.id}')
                
                # We use the name from the snapshot, fallback to 'Pet Service'
                service_name = service_snapshot.get('name', 'Pet Service')
                
                try:
                    checkout_session = StripeService.create_booking_checkout_session(
                        booking, service_name, total_item_price, success_url, cancel_url
                    )
                    ...
                    # If Stripe fails, we can either rollback or let the booking stay PENDING
                    # without a URL, so they can retry payment later. Let's let it stay PENDING for now.
                    import logging
                    logging.getLogger(__name__).error(f"Failed to generate Stripe session for booking {booking.id}: {e}")
                ...
                # 8. Invalidate Availability Cache (Atomic on Commit)
                ...
                # Inject checkout URL into the response instance dynamically
                # (You could also use a custom Response overriding `create` instead of returning the object here,
                # but adding it to the instance is simplest as the serializer will include it).
                return booking
    ...
    def payment_webhook(self, request):
        """
        Stripe webhook endpoint to handle checkout.session.completed for bookings.
        """
        payload = request.body
        sig_header = request.META.get('HTTP_STRIPE_SIGNATURE')

        if not payload or not sig_header:
            return Response({"error": "Missing payload or signature"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            try:
                ...
                # Could be a subscription payment instead, let the other webhook handle it, or log it
                import logging
                logging.getLogger(__name__).warning(f"Booking missing for Stripe session: {transaction_id}")
...
class VisitGroupViewSet(viewsets.ModelViewSet):
    queryset = VisitGroup.objects.all()
    serializer_class = VisitGroupSerializer
    permission_classes = [permissions.IsAuthenticated]


from .models import SubscriptionPlan, ProviderSubscription, SubscriptionPayment
from .serializers import SubscriptionPlanSerializer, ProviderSubscriptionSerializer

class SubscriptionPlanViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Read-only viewset for providers to view available subscription plans.
    """
...
class ProviderSubscriptionViewSet(viewsets.ModelViewSet):
    """
    ViewSet for providers to manage their subscriptions and view billing.
    """
    ...
    def get_queryset(self):
        user = self.request.user
        provider_id = getattr(user, 'id', None) or getattr(user, 'auth_user_id', None)
        return ProviderSubscription.objects.filter(provider_id=provider_id)

    @action(detail=False, methods=['post'])
    def subscribe(self, request):
        """
        Subscribe or upgrade to a plan.
        Expects: {"plan_id": "UUID"}
        """
        user = self.request.user
        provider_id = getattr(user, 'id', None) or getattr(user, 'auth_user_id', None)
        
        if not provider_id:
            return Response({"error": "Unable to identify provider ID"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        with transaction.atomic():
            ...
            if plan.trial_days > 0:
                ...
                status_val = 'PENDING' # Will be active after payment
                if plan.billing_cycle == 'YEARLY':
                    end_date = start_date + timedelta(days=365)
                else:
                    end_date = start_date + timedelta(days=30)

            # Create the subscription in PENDING/TRIAL state
            subscription = ProviderSubscription.objects.create(
                provider_id=provider_id,
                plan=plan,
                status=status_val,
                start_date=start_date if status_val == 'TRIAL' else None,
                end_date=end_date if status_val == 'TRIAL' else None,
                trial_end_date=trial_end_date,
                auto_renew=True
            )

            # Generate Stripe Checkout Session
            success_url = self.request.build_absolute_uri('/subscription/success/')
            cancel_url = self.request.build_absolute_uri('/subscription/cancel/')
            checkout_session = StripeService.create_subscription_checkout_session(plan, success_url, cancel_url)
    ...
    def cancel(self, request, pk=None):
        ...
        if subscription.status not in ['ACTIVE', 'TRIAL']:
            return Response({"error": "Only active or trial subscriptions can be cancelled"}, status=status.HTTP_400_BAD_REQUEST)

        subscription.auto_renew = False
        subscription.save()
        
        return Response({"message": "Subscription will not auto-renew at the end of the billing cycle."})

    @action(detail=False, methods=['post'], permission_classes=[permissions.AllowAny])
    def webhook(self, request):
        """
        Webhook to handle payment success from Stripe/Razorpay.
        Expects: {"transaction_id": "string", "status": "SUCCESS|FAILED"}
        """
        transaction_id = request.data.get('transaction_id')
        payment_status = request.data.get('status')

        if not transaction_id or payment_status != 'SUCCESS':
            return Response({"error": "Invalid webhook payload"}, status=status.HTTP_400_BAD_REQUEST)

        try:
            payment = SubscriptionPayment.objects.get(transaction_id=transaction_id, status='PENDING')
        ...
        with transaction.atomic():
            ...
            # 2. Update Subscription Status and activate dates
            subscription = payment.subscription
            
            # If it was PENDING (no trial), activate it now
            ...
            # If it was a renewal or already TRIAL, just ensure status is ACTIVE
            elif subscription.status == 'TRIAL':
                # Trial doesn't immediately become ACTIVE until trial ends, 
                # but we've secured the payment method.
                pass 
