...
class SlotLockService:
    ...
    @staticmethod
    def _generate_key(identifier, start_datetime):
        """Identifier can be emp:id, pet:id, or res:id"""
        return f"slot_lock:{identifier}:{start_datetime.isoformat()}"

    @staticmethod
    def lock_slot(identifier, start_datetime):
        try:
            r = SlotLockService.get_redis_client()
            key = SlotLockService._generate_key(identifier, start_datetime)
            success = r.set(
                key,
                "locked",
                nx=True,
    ...
    @staticmethod
    def release_slot(identifier, start_datetime):
        try:
            r = SlotLockService.get_redis_client()
            key = SlotLockService._generate_key(identifier, start_datetime)
            r.delete(key)
    ...
    def lock_multiple(locks):
        ...
        'locks' is a list of (identifier, start_datetime).
        ...
        try:
            for identifier, dt in locks:
                key = SlotLockService._generate_key(identifier, dt)
                success = r.set(
                    key,
                    "locked",
                    nx=True,
    ...
    def release_multiple(locks):
        ...
        keys = [SlotLockService._generate_key(identifier, dt) for identifier, dt in locks]
        try:
            r = SlotLockService.get_redis_client()
            r.delete(*keys)
        except Exception as e:
