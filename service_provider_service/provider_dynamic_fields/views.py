# # provider_dynamic_fields/views.py
...
#         try:
#             response = requests.get(f"{SUPERADMIN_URL}?target={target}", timeout=5)
...
#         serializer = ProviderFieldValueSerializer(obj)
#         return Response(serializer.data)


# # ----------------------------------------------------------
# # 5) UPDATE SINGLE FIELD VALUE
# # ----------------------------------------------------------
...
class ProviderDynamicFieldsView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        try:
            resp = requests.get(f"{SUPERADMIN_URL}?target={target}", timeout=5)
